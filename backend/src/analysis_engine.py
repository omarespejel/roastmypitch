...
class PitchDeckAnalyzer:
    def __init__(self):
        self.vc_rubric = {
            'team': {
                'keywords': ['founder', 'team', 'experience', 'background', 'CEO', 'CTO', 'leadership'],
            ...
            'market': {
                'keywords': ['TAM', 'market size', 'billion', 'opportunity', 'market', 'addressable'],
            ...
            'traction': {
                'keywords': ['users', 'revenue', 'growth', 'MRR', 'customers', 'metrics', 'retention'],
            ...
            'economics': {
                'keywords': ['CAC', 'LTV', 'unit economics', 'margin', 'profitability', 'monetization'],
            ...
            'competition': {
                'keywords': ['competitors', 'competitive', 'advantage', 'differentiation', 'moat'],
            ...
            'problem': {
                'keywords': ['problem', 'pain point', 'challenge', 'issue', 'frustration'],
        ...
        self.pm_rubric = {
            'persona': {
                'keywords': ['user persona', 'JTBD', 'customer profile', 'target user', 'user research'],
            ...
            'problem': {
                'keywords': ['problem', 'pain point', 'user frustration', 'workflow', 'friction'],
            ...
            'solution': {
                'keywords': ['solution', 'feature', 'product', 'functionality', 'user experience'],
            ...
            'metrics': {
                'keywords': ['metrics', 'KPI', 'engagement', 'retention', 'conversion', 'success'],
            ...
            'roadmap': {
                'keywords': ['roadmap', 'timeline', 'milestone', 'development', 'iteration'],
    ...
    def analyze_document_gaps(self, content: str, agent_type: AgentType) -> AnalysisResult:
        ...
        for section_id, section_data in rubric.items():
            ...
            keywords_found = sum(1 for keyword in section_data['keywords'] 
            ...
            if keywords_found < 2:  # Threshold for "covered"
                ...
                suggested_actions.append({
                    ...
                    'priority': 'high' if section_id in ['team', 'market', 'traction', 'problem'] else 'medium'
                })
    ...
    def _generate_next_steps(self, missing_sections: List[str], agent_type: AgentType) -> List[str]:
        if agent_type == AgentType.SHARK_VC:
            priorities = ['team', 'market', 'traction', 'economics', 'competition', 'problem']
        else:
        ...
        for priority in priorities:
            if priority in missing_sections:
                next_steps.append(f"Focus on {priority} - this is critical for {agent_type.value}")
