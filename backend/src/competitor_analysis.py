...
class CompetitorAnalyzer:
    ...
    async def analyze_competitors(
        self, project_description: str, starknet_focus: bool = True
    ) -> Dict:
        """Analyze competitors for a given project in the Starknet ecosystem"""

        scope = (
            "Starknet ecosystem and broader blockchain/DeFi space"
            if starknet_focus
            else "blockchain and traditional tech space"
        )
        ...
        try:
            ...
            return {
                ...
                "direct_competitors": self._extract_competitors(
                    response.text, "direct"
                ),
                "indirect_competitors": self._extract_competitors(
    ...
    def _extract_competitors(self, text: str, competitor_type: str) -> List[Dict]:
        ...
        for line in lines:
            if any(
                word in line.lower()
                for word in ["competitor", "alternative", "project"]
            ):
    ...
    def _extract_advantages(self, text: str) -> List[str]:
        ...
        for line in lines:
            if any(
                ...
                for phrase in ["advantage", "strength", "benefit", "moat"]
            ):
                if "-" in line or "•" in line:
